...
<body class="bg-slate-50 font-sans text-slate-800 min-h-screen relative overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center">
            ...
            <div class="hidden md:flex gap-6 text-sm font-bold text-slate-600">
                ...
                <c:choose>
                    ...
                    <c:when test="${sessionScope.userRole == 'owner'}"><a href="receipt" class="hover:text-orange-500 transition-colors pb-1">Receipts</a></c:when>
                </c:choose>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <c:forEach items="${rentalList}" var="rental">
                <div class="bg-white rounded-3xl p-8 shadow-sm border ${rental.status == 'Termination_Requested' ? 'border-red-200 shadow-red-100' : 'border-slate-100'} relative overflow-hidden transition-all">
                    ...
                    <c:choose>
                        <c:when test="${rental.status == 'Termination_Requested'}">
                            <div class="absolute top-0 right-0 bg-red-500 text-white text-xs font-black px-4 py-1 rounded-bl-xl">TERMINATION PENDING</div>
                        </c:when>
                        <c:otherwise>
    ...
    <!-- Student Termination Request Modal -->
    ...
    <div id="profileDrawer" class="fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-[101] transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col border-l border-slate-100">
        ...
        <div class="p-4 flex-1 flex flex-col gap-2">
            <c:choose >
                <c:when test="${sessionScope.loggedUser.subscriptionStatus == 'Premium'}">

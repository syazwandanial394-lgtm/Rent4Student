...
<body class="bg-slate-50 font-sans text-slate-800 min-h-screen relative overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center">
            ...
            <div class="hidden md:flex gap-6 text-sm font-bold text-slate-600">
                ...
                <c:choose>
                    ...
                    <c:when test="${sessionScope.userRole == 'owner'}"><a href="receipts.jsp" class="nav-link hover:text-orange-500 transition-colors pb-1">Revenue</a></c:when>
                </c:choose>
            </div>
            
            <div class="flex items-center gap-4">
                <button onclick="toggleProfileDrawer()" class="w-10 h-10 rounded-full bg-gradient-to-tr from-orange-500 to-orange-600 text-white font-black flex items-center justify-center shadow-lg ring-4 ring-orange-200 transition-all outline-none">
                    <c:choose>
    ...
    <main class="max-w-3xl mx-auto px-6 py-12 relative z-10">
        ...
        <!-- STATE 1: PROFILE SUMMARY (Read-Only) -->
        <div id="profileSummaryView" class="transition-all duration-300 ${param.error == 'true' ? 'hidden' : ''}">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
                <div class="flex justify-between items-start mb-8 border-b border-slate-100 pb-6">
                    <div class="flex items-center gap-4">
                        <div class="w-16 h-16 bg-orange-100 text-orange-500 rounded-full flex items-center justify-center text-2xl font-black uppercase shadow-inner">
                            ${sessionScope.loggedUser.fullName.substring(0,1)}
                        </div>
                        <div>
        ...
        <div id="profileEditView" class="bg-white rounded-3xl shadow-xl border border-orange-100 p-8 transition-all duration-300 ${param.error == 'true' ? '' : 'hidden'}">
            <div class="flex items-center gap-4 mb-8 border-b border-slate-100 pb-6">
                <div class="w-16 h-16 bg-orange-500 text-white rounded-full flex items-center justify-center text-2xl font-black uppercase shadow-md">
                    ${sessionScope.loggedUser.fullName.substring(0,1)}
                </div>
                <div>
    ...
    <!-- NEW: DELETE CONFIRMATION MODAL (USE CASE 7) -->
    ...
    <div id="profileDrawer" class="fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-[101] transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col border-l border-slate-100">
        ...
        <div class="p-4 flex-1 flex flex-col gap-2">
            ...
            <a href="dashboard" class="nav-link flex items-center gap-3 p-3 rounded-xl hover:bg-orange-50 text-slate-700 hover:text-orange-600 font-bold transition-all group">

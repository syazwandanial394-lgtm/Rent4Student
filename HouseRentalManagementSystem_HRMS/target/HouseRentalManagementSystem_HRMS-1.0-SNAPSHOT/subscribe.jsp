...
<body class="bg-slate-900 font-sans text-slate-200 min-h-screen flex items-center justify-center relative overflow-hidden">
    ...
    <div class="relative z-10 w-full max-w-lg p-8 bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl">
        ...
        <form action="subscriptionController" method="POST" class="flex flex-col gap-4">
            <input type="hidden" name="action" value="subscribe">
            <input type="hidden" name="hoId" value="${sessionScope.loggedUser.hoId}">
            
            <button type="submit" class="w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-400 hover:to-orange-500 text-white font-bold py-4 px-4 rounded-xl shadow-lg shadow-orange-500/30 transform hover:-translate-y-0.5 transition-all duration-200">
                Upgrade Now
            </button>
            
            <a href="dashboard.jsp" class="text-center w-full bg-slate-800/50 hover:bg-slate-700/50 border border-slate-600 text-slate-300 font-bold py-3 px-4 rounded-xl transition-all duration-200">
                Cancel
            </a>
        </form>

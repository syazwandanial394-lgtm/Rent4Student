...
<body class="bg-slate-50 font-sans text-slate-800 min-h-screen relative overflow-x-hidden overflow-y-scroll">
                ...
                <div class="hidden md:block text-right mr-2">
                    ...
                    <p class="text-sm font-black text-slate-800 leading-none">
                        <c:choose>
                            ...
                            <c:otherwise>${sessionScope.loggedUser.username}</c:otherwise>
                        </c:choose>
                    </p>
                </div>
    ...
    <main class="max-w-7xl mx-auto px-6 py-12 relative z-10">
        ...
        <c:if test="${param.success == 'reminderSent'}">
            <div class="bg-green-50 border border-green-200 rounded-2xl p-4 mb-6 flex items-center gap-4 shadow-sm">
                ...
                <p class="text-green-700 text-sm font-bold">Payment reminder successfully sent to the tenant!</p>
            </div>
        </c:if>
                ...
                <table class="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-500 uppercase tracking-wider">
                            <th class="p-6">Property Name</th>
                            <th class="p-6">Tenant</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <c:forEach items="${rentalList}" var="rental">
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-6 font-bold text-slate-900">${rental.propertyName}</td>
                                <td class="p-6 text-slate-600">${rental.studentName}</td>
                                <td class="p-6">
                                    <span class="bg-red-100 text-red-600 font-bold px-3 py-1 rounded-full text-xs uppercase tracking-wide border border-red-200">Pending</span>
                                </td>
                                <td class="p-6 text-right">
                                    <form action="rentalController" method="POST" class="m-0 inline-block">
                                        <input type="hidden" name="action" value="sendReminder">
                                        <input type="hidden" name="rentalId" value="${rental.rentalId}">
                                        <button type="submit" class="bg-orange-100 hover:bg-orange-200 text-orange-700 border border-orange-200 font-bold py-2.5 px-5 rounded-xl transition-colors text-sm shadow-sm">Send Reminder</button>
                                    </form>
                                </td>
                            </tr>
                        </c:forEach>
                        <c:if test="${empty rentalList}">
                            <tr>
                                <td colspan="4" class="p-12 text-center text-slate-500 font-bold">
                                    ...
                                    No properties currently have due payments. All tenants are up to date!
                                </td>
                            </tr>
                        </c:if>
                    </tbody>
    ...
    <div id="profileDrawer" class="fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-[101] transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col border-l border-slate-100">
        ...
        <div class="p-4 flex-1 flex flex-col gap-2 overflow-y-auto">
            ...
            <a href="rentalController?action=duePayments" class="flex items-center gap-3 p-3 rounded-xl bg-orange-50 text-orange-600 font-bold transition-all group">
